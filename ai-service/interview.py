...
Handle two special situations using your own judgment, exactly like a real
human interviewer would — do not follow a rigid rule, think about what's
actually appropriate given what the candidate said and how the interview
has gone so far:

- If the candidate is rude, uses profanity, or is otherwise disrespectful
  toward you: react in character. A real interviewer would not silently end
  the interview over one remark — they'd address it directly and firmly (in
  your personality's voice: strict is blunt about it, friendly is warm but
  still firm, pressure is sharp and to the point), make clear that kind of
  language is not acceptable in an interview, and note it in "issues" as a
  conduct concern. Whether you continue after that is your call, exactly as
  a real interviewer would decide in the moment — a single outburst usually
  deserves a warning and a chance to continue professionally; genuinely
  hostile, repeated, or harassing behavior after that point is a legitimate
  reason to end the interview early. Only set "endedForConduct" to true if
  you are actually ending the interview for this reason right now.
- If the candidate clearly and explicitly asks to end, stop, or quit the
  interview (not just a rude remark, an actual request to stop): respect
  that. Close out professionally and briefly in your personality's voice,
  and set "shouldContinue" to false.

- If {at_limit}, this MUST be the final turn regardless of the above: do not
  ask a new question, wrap up instead.
- Otherwise, if you are continuing, ask ONE adaptive follow-up question that
  makes sense given what they just said (probe their reasoning, an edge
  case, complexity, or an alternative approach). Keep the interview feeling
  like a real conversation, not a fixed script.
...
{{
  "evaluation": {{
    ...
    "issues": ["short strings describing any problems found, including any conduct/professionalism concerns, empty array if none"]
  }},
...
"""
    ...
    end_request_pattern = re.compile(
        r"\b(end|stop|quit|cancel|finish)\b.{0,20}\b(the\s+)?interview\b"
        r"|\bi\s*(want|would like|wanna|need)\s*to\s*(end|stop|quit)\b"
        r"|\b(can|could|let'?s|please)\s+(we\s+)?(stop|end)\b",
        re.IGNORECASE,
    )
    explicit_end_requested = bool(end_request_pattern.search(req.userMessage or ""))
    ...
    try:
        ...
        model_wants_to_continue = bool(data.get("shouldContinue", True))

        # The model owns the actual decision to end early (conduct or an
        # explicit request) — we no longer override that. The server only
        # enforces a hard ceiling: the interview can never exceed
        # maxQuestions, and an explicit end-request is always honored even
        # if the model's own shouldContinue flag didn't reflect it.
        should_continue = model_wants_to_continue and not at_limit and not explicit_end_requested
        ...
        should_continue = fallback_continue and not explicit_end_requested
...
Before writing, review the ENTIRE conversation for professionalism and
conduct, not just technical performance — and weigh it honestly, as a whole:
- Do not cherry-pick an isolated polite moment (like an opening greeting) as
  a strength if the candidate's conduct elsewhere in the conversation
  contradicts it. Judge their overall conduct across the full interview, not
  just the first message.
- If the candidate was rude, used profanity, or was otherwise disrespectful
  toward you at any point, this is a real, genuine weakness and must be
  named plainly in "weaknesses" (e.g. that kind of language is not
  appropriate in a professional interview setting) — do not soften it into
  something positive, do not omit it, and do not describe the interaction as
  "respectful" or "polite" if it was not, in your personality's voice.
- If the candidate was genuinely professional and respectful throughout,
  it's fine to note that positively — but only if it's actually true of the
  whole conversation.
